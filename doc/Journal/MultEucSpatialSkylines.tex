...
We implemented and tested 4 different CPU approaches to compare with our GPU proposal. Single thread, single thread discarding, single thread sorting and multi thread.\newline
...
\textbf{Multi thread discarding:} If a point $p_i$ is skyline or not, does not affect the result of $p_j, j \neq i$ to be skyline or not. Thus, it is easy to imagine how we can implement the discarding approach in multiple threads. We set up a maximum number of threads and each thread is responsible of computing the skylines of a subset of $P$. We keep the list of skylines updates in a thread safe structure.\newline
...
Our proposal, the GPU algorithm, shows a penalty for smaller sets. The GPU additional steps like uploading to GPU memory or launching the kernel, reduces the performance such that is a bad approach for small sets. As well as the multi thread discarding, when the input data grows, the performance of the GPU approach does too. We observe an improvement up to 89\% for bigger sets.

Note that, in the GPU approach, we considered the total time taken to compute the skylines as the time to upload to GPU memory, plus the time to compute the skyline, plus download the result back to CPU.
