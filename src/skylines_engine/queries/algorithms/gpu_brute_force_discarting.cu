...
__device__ void _ComputePartialSkylineDiscarting(
    ...
    bool is_skyline = global_pos < input_p_size && result[global_pos] == 0;
...
__global__ void ComputePartialSkylineDiscarting(
    const sl::queries::data::WeightedPoint *input_p,
    size_t input_p_size,
    int input_q_size,
    ...
    case sl::queries::algorithms::DistanceType::Nearest:
        _ComputePartialSkylineDiscarting(input_p, input_p_size, input_q_size, NearestFunc, statistics, result);
        break;
    case sl::queries::algorithms::DistanceType::Furthest:
        _ComputePartialSkylineDiscarting(input_p, input_p_size, input_q_size, FurthestFunc, statistics, result);
        break;
    default:
        break;
    }
...
extern "C" void ComputeGPUSkylineDiscarting(
    ...
    */
    ...
    cudaError_t e = cudaMemsetAsync(result_d(), 0, input_p_size_SHARED_MEM_SIZE_multiple * sizeof(float), gpu_stream());
    if (e != cudaSuccess) {
        std::cout << cudaGetErrorString(e) << '\n';
    }

    ComputePartialSkylineDiscarting<<<grid, threadsPerBlock, 0, gpu_stream()>>>(input_p_d(), input_p_size, input_q_size, distance_type, statistics_d(), result_d());
    e = cudaGetLastError();
    if (e != cudaSuccess) {
        std::cout << cudaGetErrorString(e) << '\n';
    }
